...
/**
 * Gera o HTML standalone do laudo (usado pela rota /api/print e pela
 * sincronização com o Google Drive via Puppeteer). Retorna null se o exame
 * ou o laudo não existirem.
 */
export async function buildReportHtml(examId: string): Promise<string | null> {
    ...
    const findings = safeParse(exam.report.findings);
    const conditions: any = (exam.report.diagnosticConditions as any) || {};
    const selectedImages: any = (exam.report as any).selectedImages || {};

    const selectedImgs = exam.images.filter(
        img => img.id === selectedImages?.od || img.id === selectedImages?.oe
    );

    const imagesHtml = selectedImgs.length > 0
        ? selectedImgs.map(img => `
              <div class="border rounded-lg overflow-hidden p-2 bg-gray-50">
                ...
                <p class="text-[8px] font-bold text-gray-400 uppercase mt-2 text-center">${img.id === selectedImages?.od ? 'Olho Direito (OD)' : 'Olho Esquerdo (OE)'}</p>
              </div>
            `).join('')
      ...
      <div class="max-w-4xl mx-auto space-y-8">
        ...
        <div class="pt-10 border-t flex justify-between items-end">
          ...
          <div class="text-right">
             ...
             <p class="text-[9px] font-bold text-gray-400 uppercase">CRM: ${exam.report.doctorCRM || '—'}</p>
          </div>
        </div>
      </div>
    </body>
